...
//2. 如果函数声明有默认值，函数实现的时候就不能有默认参数（即声明和实现只能有一个默认参数）
...
public:
	...
	//前置++
    //返回值一定是引用返回，为了递增时在同一个对象进行递增，比如说++(++myInt)
	MyInteger& operator++() {
		//先++
		m_Num++;
		//再返回
	...
	//后置++
    //返回值一定是值返回，因为如果是引用返回，那么返回的是一个局部对象temp，那么出了作用域后局部对象被释放，下面都是非法操作
	MyInteger operator++(int) { //int是占位参数，让编译器知道发生了函数重载
		//先返回
