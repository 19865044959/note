...
-   拓展：public、protected、private继承有什么区别吗？
    ...
    -   protected继承：基类的public成员的访问权限在子类中变成了protected，其他不变（即：派生类可以通过**成员函数**访问继承来的基类的public与protected变量，但是无权访问private变量；派生类类外无法访问基类的public与protected，因为派生类继承下来时，已经变成了protected了）
...
-   原因：因为基类中private变量对于派生类来说是不可访问的，因此是不允许在派生类构造函数中直接初始化基类的private变量的。但是基类的private变量又是现实存在的，为了解决这个矛盾，所以每次调用派生类构造函数时，派生类构造函数中调用基类构造函数，委托基类去初始化基类成员变量（有委托构造函数那味儿了）
...
<img src="../../pictures/image-20210523195640993.png" alt="image-20210523195640993" style="zoom:80%;" />
...
-   自动类型推导 auto与decltype
    ...
    -   auto：自动推导变量类型
    -   decltype：自动推导表达式类型，比如说我上面有了迭代器iter1，我想再声明一个迭代器iter2，那么可以这样写：`decltype(iter1) iter2` 不用再麻烦的写： `vector<int> :: iterator iter2`了
-   范围for循环，类似python一样，一般与auto用在一起，for(auto i : vec)，对于vec里面的每一个变量i
-   初始化列表
    -   利用初始哈列表对成员变量进行初始化，用初始化列表的好处有很多，比如说可以初始化const &关键字的成员变量，而不能直接先默认初始化再赋值给这些变量
-   委托构造函数
    -   定义：一个委托构造函数使用它所属类的其他构造函数执行它自己的初始化过程。
    -   理解：有点儿像在派生类初始化时调用基类构造函数来帮助派生类初始化继承而来的基类成员（因为派生类构造函数不允许直接访问继承而来的基类private变量）
-   无序容器
...
-   就起作用的阶段而言： #define 是在编译的预处理阶段（预编译）起作用，而const是在 编译、运行的时候起作用。
